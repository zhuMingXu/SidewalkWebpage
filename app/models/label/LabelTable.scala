...
object LabelTable {
  ...
  def countNonResearcherLabels: Int = db.withSession { implicit session =>
    val thing = for {
      (_labels, _tasks) <- labelsWithoutDeleted.innerJoin(auditTasks).on(_.auditTaskId === _.auditTaskId)
    } yield _tasks.userId
    thing.filterNot(_ inSet researcherIdsSansAnon).list.size
  }

  def countRegisteredUserSansResearcherLabels: Int = db.withSession { implicit session =>
    val thing = for {
      (_labels, _tasks) <- labelsWithoutDeleted.innerJoin(auditTasks).on(_.auditTaskId === _.auditTaskId)
    } yield _tasks.userId
    thing.filterNot(_ inSet researcherIds).list.size
  }

  def countAnonUserLabels: Int = db.withSession { implicit session =>
    val tasks = auditTasks.filter(_.userId === anonUserId)
    val thing = for {
      (_labels, _tasks) <- labelsWithoutDeleted.innerJoin(tasks).on(_.auditTaskId === _.auditTaskId)
    } yield _tasks.userId
    thing.list.size
  }
  ...
  def countNonResearcherLabelsBasedOnType(labelTypeString: String): Int = db.withSession { implicit session =>
    val thing = for {
      (_labels, _tasks) <- labelsWithoutDeleted.innerJoin(auditTasks).on(_.auditTaskId === _.auditTaskId)
    } yield (_tasks.userId, _labels.labelTypeId)

    val thing2 = thing.filterNot(_._1 inSet researcherIdsSansAnon)
    thing2.filter(_._2 === LabelTypeTable.labelTypeToId(labelTypeString)).list.size
  }

  def countRegisteredUserSansResearcherLabelsBasedOnType(labelTypeString: String): Int = db.withSession { implicit session =>
    val thing = for {
      (_labels, _tasks) <- labelsWithoutDeleted.innerJoin(auditTasks).on(_.auditTaskId === _.auditTaskId)
    } yield (_tasks.userId, _labels.labelTypeId)

    val thing2 = thing.filterNot(_._1 inSet researcherIds)
    thing2.filter(_._2 === LabelTypeTable.labelTypeToId(labelTypeString)).list.size
  }

  def countAnonUserLabelsBasedOnType(labelTypeString: String): Int = db.withSession { implicit session =>
    val thing = for {
      (_labels, _tasks) <- labelsWithoutDeleted.innerJoin(auditTasks).on(_.auditTaskId === _.auditTaskId)
      if _tasks.userId === anonUserId
    } yield _labels.labelTypeId

    thing.filter(_ === LabelTypeTable.labelTypeToId(labelTypeString)).list.size
  }

  def getPercentageOfLabelsBasedOnType(labelTypeString: String): String = db.withTransaction(implicit session =>
    "%.1f".format(100.0 * countLabelsBasedOnType(labelTypeString).toFloat / countLabels)
  )
  def getPercentageOfNonResearcherLabelsBasedOnType(labelTypeString: String): String = db.withTransaction(implicit session =>
    "%.1f".format(100.0 * countNonResearcherLabelsBasedOnType(labelTypeString).toFloat / countNonResearcherLabels)
  )
  def getPercentageOfRegisteredUserSansResearcherLabelsBasedOnType(labelTypeString: String): String = db.withTransaction(implicit session =>
    "%.1f".format(100.0 * countRegisteredUserSansResearcherLabelsBasedOnType(labelTypeString).toFloat / countRegisteredUserSansResearcherLabels)
  )
  def getPercentageOfAnonUserLabelsBasedOnType(labelTypeString: String): String = db.withTransaction(implicit session =>
    "%.1f".format(100.0 * countAnonUserLabelsBasedOnType(labelTypeString).toFloat / countAnonUserLabels)
  )
