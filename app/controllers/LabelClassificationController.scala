...
/**
  ...
  * @param env
  */
class LabelClassificationController @Inject() (implicit val env: Environment[User, SessionAuthenticator])
  ...
  def index = UserAwareAction.async { implicit request =>
    if(isAdmin(request.identity)){
      Future.successful(Ok(views.html.labelclassification("Label Classification")))
    }
    else {
      request.identity match{
        case Some(user) => Future.successful(Ok(views.html.signIn(SignInForm.form, "/labelclassification", Some(user))))
        case None => Future.successful(Ok(views.html.signIn(SignInForm.form, "/labelclassification")))
      }
